...
class Legra:
    ...
    @classmethod
    def load(cls, path: str | Path) -> "Legra":
        """
        ...
        # 4. indexer  (reuse if stored, else build) ----------------------
        ...
        if faiss_file.exists():
            ...
            indexer.build_index(emb_matrix)  # quick — in RAM only

        # 5. retriever  (always present)  -------------------------------
        from .retrieval.neighbor_retriever import NeighborRetriever

        retriever: Retriever = NeighborRetriever(
                embedder=embedder, indexer=indexer, docs_meta=docs_meta
                )

        # 6. communities & summaries  -----------------------------------
        ...
        # 8. create instance & inject loaded state -----------------------
        return cls(
    ...
    def query(self, query_text: str, top_k: int = 5, generate: bool = True, **gen_kwargs) -> str:
        ...
        """
        if self.retriever is None:
            raise RuntimeError("Retriever is not configured. Call index() first.")
